...
library(stringr)
library(parallel)
library(textclean) ## remove non-ascii characters that Rlibstree introduces
...
########## 
tir$tirseq=sapply(1:nrow(tir), function(x) getLongestCommonSubstring(c(tir$upstreamExtra[x], tir$downstreamExtraRC[x]), as.character=T))
tir$tirseqRC=sapply(1:nrow(tir), function(x) getLongestCommonSubstring(c(tir$upstreamExtraRC[x], tir$downstreamExtra[x]), as.character=T))

### remove non-ascii characters that Rlibstree introduces - DO NOT KNOW IF THESE ARE COMPREHENSIVE!!!!!
## check file with head(replace_non_ascii(tir$tirseq, replacement='', remove.nonconverted=T)), then look at individual line that fails in tir$tirseq[[x]]
tir$tirseq=lapply(tir$tirseq, function(x) gsub('\xc1', '', x, useBytes=T))
tir$tirseq=lapply(tir$tirseq, function(x) gsub('\xd1', '', x, useBytes=T))
tir$tirseq=lapply(tir$tirseq, function(x) gsub('!', '', x))
tir$tirseq=lapply(tir$tirseq, function(x) gsub('1', '', x))

tir$tirseq=mclapply(tir$tirseq, function(x) replace_non_ascii(x, replacement='', remove.nonconverted=T), mc.cores=ncores)		  
